...
class RateioListScreen extends StatefulWidget {
  createState() => _RateioListAdmState();
}
...
class _RateioListAdmState extends State<RateioListScreen> {
  ...
  final RateioBloc _rateioBloc = RateioBloc();

  //Allows rateios to be dismissable horizontally
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        ...
        actions: [
          IconButton(
              ...
              onPressed: () {
                Messages().showAlertDialog(context, "Rateio",
                    "Divisão proporcional entre os produtos/serviços que se utilizam de um mesmo processo/insumo.\nEx.:\nNome: Energia elétrica\nValor total: R\$ 150,00",
                    buttonText: "Entendi");
              }),
          IconButton(
              icon: Icon(
      ...
      floatingActionButton: new FloatingActionButton(
        ...
        onPressed: () {
          ScreenNavigator().newRateioScreen(context, _rateioBloc);
        },
  ...
  Widget buildRateioList(BuildContext context, int index, List<dynamic> list) {
    Rateio rateio = list[index];
  ...
  Widget getRateiosWidget() {
    /*The StreamBuilder widget,
    basically this widget will take stream of data (rateios)
    and construct the UI (with state) based on the stream
    */
    return StreamBuilder(
      stream: _rateioBloc.rateios,
      builder: (BuildContext context, AsyncSnapshot<List<Rateio>> snapshot) {
        return getRateioWidget(snapshot);
      },
    );
  }

  Widget getRateioWidget(AsyncSnapshot<List<Rateio>> snapshot) {
    ...
    if (snapshot.hasData) {
      ...
      */
      return snapshot.data.length != 0
          ...
          : Container(
              child: Center(
              //this is used whenever there 0 rateios
              //in the data base
